...
public class AuthReqBuilder {
    ...
    private static final Log log = LogFactory.getLog(AuthReqBuilder.class);
    ...
    public AuthnRequest buildAuthenticationRequest(String issuerId, String destination, String acsUrl,
            boolean isPassive, int tenantId, String tenantDomain, String nameIdPolicy) throws Exception {
        Util.doBootstrap();
        AuthnRequest authnRequest = (AuthnRequest) Util.buildXMLObject(AuthnRequest.DEFAULT_ELEMENT_NAME);
        authnRequest.setID(Util.createID());
        authnRequest.setVersion(SAMLVersion.VERSION_20);
     ...
     */
    public static AuthnRequest setSignature(AuthnRequest authnRequest, String signatureAlgorithm, X509Credential cred)
            throws Exception {
        try {
